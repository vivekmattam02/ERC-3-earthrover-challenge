...
## Field Status Override (2026-07-16)

The pipeline is implemented, but the field result is not a reliable autonomous
repeat yet. Recent front-camera-only rough-terrain tests sent commands and
physically moved the rover, while localization repeatedly remained near startup
steps and entered scan/probe loops without sustained graph-step progression.

Therefore:

- `smoke_run01_c` is the best current **coverage/reference candidate**, not a
  proven operational or deployment route;
- `src/adaptive_pursuit_controller.py` is the active rough-terrain controller
  family selected by `scripts/run_prepared_route.py --rough-terrain`;
- rough-terrain launches now default to no route-heading guidance because its
  compass/route relation was unreliable; use `--use-route-heading` only for a
  deliberately validated route;
- manual bags do not contain teleop control labels, so exact control replay is
  not available from the existing recordings.

For the detailed evidence and the required validation gate, read
`obsidian_vault/01 Source of Truth/No-GPS Field Trial - Findings.md`.
Later historical sections in this file are preserved for the project story;
this section takes precedence when they conflict.

## Workspace Consolidation (2026-07-16)

Do not treat every visible Python file or local clone as an active controller.
The maintained live route-repeat path is:

`scripts/run_prepared_route.py` -> `live_indoor_runtime.py` ->
`src/adaptive_pursuit_controller.py`.

- `legacy/` contains superseded indoor runtime/controller snapshots only.
- `mbra_repo/` is used only for deliberate `--controller mbra` experiments.
- `mbra_repo_1/` is an older image-memory reference with no model weights.
- `nyu-earthrover-main/` is an ignored baseline clone; the actual local SDK is
  `earth-rovers-sdk/`.
- `obsidian_vault/` is a separate Git repository and owns the curated project
  narrative and field evidence.
- `.claude/` stores local tool permissions; it is not project evidence.
...
1. `scripts/record_sdk_session.py`
   ...
   - front-only by default; it does not capture teleop commands
...
9. `src/adaptive_pursuit_controller.py`
   - current rough-terrain controller family
   - selected by the prepared-route launcher in `--rough-terrain` mode
   - implemented but not field-validated as a reliable route follower

10. `src/local_controller.py`
   - earlier simple-controller baseline
   - retained as a comparison and fallback implementation
...
Later unreviewed recording:

- `recordings/run_05_clean_teach.h5`
  - saved successfully
  - not yet post-processed or accepted as a route reference
...
- `data/manual_routes/run_01_1521_pp_v3`

Current comparison recommendation:

- use `smoke_run01_c` as the strongest coverage/reference candidate
- do **not** use `smoke_run02_c` as the primary route
- keep `smoke_run03_c` only as a secondary comparison route
- keep `run_01_1521_pp_v3` as the strongest **post-processed comparison route**
  so far, but do **not** replace `smoke_run01_c` with it yet
- do **not** label any of these packages as an autonomous deployment route
  until a controlled hardware-in-the-loop repeat demonstrates sustained
  progression

Current route-comparison verdict:

- `smoke_run01_c`
  - strongest current route-coverage/reference package
  - `126` extracted frames
  - `target_step=125`
  - not field-proven as a route-repeat deployment package
- `run_01_1521_pp_v3`
  - important proof that post-processing is moving in the right direction
  - but still too short / sparse to become the main route
  - `15` extracted frames
  - `target_step=14`
  - useful as a comparison artifact, not yet the main deployment route

Most important corrected assumption:

- the best teach route is **not** the cleanest-looking contiguous clip
- for this no-GPS off-road branch, route quality depends more on
  `motion-rich coverage + visual progression + relocalization evidence`
  than on static visual cleanliness alone
- post-processing therefore needs to preserve good traversal episodes, not
  just select the prettiest window

The prepared-route launcher is the correct entrypoint for supervised testing.
Its existence is not evidence that a long autonomous run is safe. Start in
dry-run, verify a visually matching physical start, then require a short
repeatable HIL segment before enabling control.

Reference launcher shape:
...
6. `src/adaptive_pursuit_controller.py`
   - current rough-terrain controller family
   - selected by the prepared-route launcher in rough-terrain mode
   - combines pursuit-style commands with confidence, tilt, RPM, and progress
     signals
   - field validation remains incomplete

7. `src/local_controller.py`
   - earlier simple local-controller baseline
   - still heuristic and retained for comparison
...
## Historical Indoor Run Reference

The following commands describe the original indoor corridor workflow. They
are not the current no-GPS rough-terrain operating procedure; consult the
field-status override at the top of this file first.
...
- `--controller simple`
  ...
  - retained for indoor/reference comparisons
  - not the controller selected by rough-terrain prepared-route runs
  - default tick rate: 2Hz, default subgoal hops: 15

## Historical Next Work: Post-Processing

Post-processing was the next planned work at the time of this section. It is
still useful, but current field evidence also requires validation of the
reference/start match and a short repeatable HIL segment.
