...
This file preserves the April working context below. The current field evidence
is more cautious: the front-camera no-GPS pipeline and command path work, but
physical rough-terrain tests did not demonstrate a reliable autonomous repeat.
Localization repeatedly stayed near startup steps and entered scan/probe loops.

Treat `smoke_run01_c` as the strongest coverage/reference candidate, not a
proven deployment route. Rough-terrain launches select
`src/adaptive_pursuit_controller.py` and default to no route-heading guidance;
the existing manual bags do not contain teleop control labels, so exact replay
is not possible. Read
`obsidian_vault/01 Source of Truth/No-GPS Field Trial - Findings.md` for the
evidence and validation gate.

The current code and field-evidence override above take precedence if any
April-era section below conflicts.
...
- `run_01` / `smoke_run01_c` is the strongest current coverage/reference route
- `run_02` is not preferred because of a dark segment and a bad tilted tail
- `run_03` is usable but weaker than run 1
- `run_01_1521_pp_v3` is the strongest current post-processed comparison route,
  but it does not replace `smoke_run01_c` yet because it is still too short /
  sparse (`target_step=14` vs `125`)
...
- verify that the physical start visually matches the reference route
- demonstrate stable graph-step progression on a short HIL segment
- only then revisit route post-processing or longer autonomous attempts

Most important corrected assumption:

- the route reference should not be chosen by visual cleanliness alone
- a stable-looking parked segment can score well visually and still be bad for
  repeat navigation
- for this terrain, the post-processor must prefer
  `motion-rich, relocalizable traversal episodes`
