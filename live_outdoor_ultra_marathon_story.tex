...
\usepackage{hyperref}
\usepackage{enumitem}
\title{Ultra Marathon Story: What We Verified, What We Changed, and Why}
\author{Outdoor Runtime Handoff}
\date{\today}
...
This is the historical GPS-marathon hardening story. Its lessons on transition
stability and not confusing ``implemented'' with ``proven'' carry into the
later no-GPS rough-terrain branch. The branch itself is distinct: it uses a
front-camera teach route and has not demonstrated a reliable autonomous repeat.
For current field truth, use
...
\item mission mode,
\item checkpoint reporting,
\item mission resume,
\item the GPS-conditioned LogoNav controller,
\item optional OSM pedestrian routing,
\item a traversability layer,
\item a semantic bias layer,
\item telemetry freeze detection,
\item and stuck recovery.
\end{itemize}

That mattered because it meant the marathon was not a controller-invention task. The real task was to make the existing outdoor stack \emph{safer and stricter} for a one-attempt long-distance run.

\section{What We Verified In The Actual Repo}
...
\item \texttt{src/earthrover\_interface.py}
\item \texttt{earth-rovers-sdk/main.py}
\item \texttt{earth-rovers-sdk/README.md}
\item \texttt{live\_outdoor\_runtime\_explained.tex}
\end{itemize}

From those files, several facts were confirmed.

First, mission mode was already real. The runtime can call the SDK \texttt{/start-mission} endpoint, fetch the official checkpoints, and later report progress through \texttt{/checkpoint-reached}. It also resumes by checking which checkpoints were already scanned.
...
\section{What The Marathon Instructions Changed}
The marathon instructions changed the engineering priorities in a very sharp way.

The organizers said, in effect:
\begin{itemize}[leftmargin=*]
\item this is a real busy area,
\item stay on sidewalks,
\item do not go to the highway,
\item be ready to intervene,
\item if the robot flips, it is over,
\item and the event consists of seven runs in series.
\end{itemize}

That immediately implied several design constraints.

The controller should be conservative. The runtime should stop on danger rather than improvise. The route should respect pedestrian structure. The system should make operator intervention easy and acceptable. And above all, it should avoid the two catastrophic failure classes: \textbf{flipping} and \textbf{leaving the sidewalk corridor}.

\section{What Was Already Good Enough To Keep}
Not everything needed to be changed.

Several parts of the outdoor system were already the right foundation:
\begin{itemize}[leftmargin=*]
\item the LogoNav outdoor controller as the main controller,
\item mission mode for checkpoint handling,
\item OSM-based pedestrian routing,
\item telemetry freeze detection,
\item and the existing recovery logic, at least in moderate form.
\end{itemize}

This led to one of the key engineering decisions of the marathon work: \textbf{do not build a new controller}. The outdoor system already had a controller path that was plausible. The higher-value work was on runtime safety and mission discipline.
...
\begin{itemize}[leftmargin=*]
\item no hard anti-flip logic integrated into the marathon profile,
\item no hard route-corridor guard to stop if the robot drifted too far off the routed pedestrian path,
\item no dedicated preflight validator script,
\item no battery warning or battery stop behavior inside the runtime,
\item and no clear documented safe-mode profile for the marathon itself.
\end{itemize}
...
\section{The First Important Principle: Do Not Overclaim}
...
Because of that, the marathon work intentionally did \emph{not} promote every existing feature into the default race profile. The approach was more conservative: keep the reliable core, add high-value guardrails, and avoid pretending that every optional perception layer was already trustworthy.

\section{What Was Added}
The implementation history was actually two-stage, and the story is clearer if it is told that way.

\subsection{Stage One: Claude's Marathon Pass}
...
\begin{itemize}[leftmargin=*]
\item a proper \texttt{--ultra-marathon} flag,
\item ultra-marathon default tightening inside \texttt{live\_outdoor\_runtime.py},
\item an IMU safety path wired into the runtime,
\item a health-gate function,
\item optional per-leg pause behavior,
\item a no-reverse marathon bias,
\item camera-loss watchdog logic,
\item operator-halt behavior after repeated recovery attempts,
\item and hardened OSM highway costs for marathon routing.
\end{itemize}
...
\subsection{Stage Two: The Narrower Safety Pass Added Here}
...
The next addition was \textbf{battery monitoring}. The runtime now emits low-battery warnings and can optionally stop when battery falls below a critical threshold.
...
\subsubsection{New File: \texttt{scripts/preflight\_marathon.py}}
...
\begin{itemize}[leftmargin=*]
\item SDK connectivity,
\item live telemetry,
\item advancing telemetry timestamp,
\item finite GPS,
\item battery threshold,
\item live camera availability,
\item LogoNav weights and config presence,
\item mission start and checkpoint retrieval,
\item and optional OSM route expansion.
\end{itemize}
...
\section{What Was Already There And Was Reused}
It is important to say clearly what was \emph{not} created from scratch in this pass.
...
\section{What The Ultra-Marathon Mode Means In Practice}
The marathon profile is intentionally conservative.

In practice, the ultra-marathon mode means:
\begin{itemize}[leftmargin=*]
\item the outdoor controller remains LogoNav,
\item pedestrian routing is expected,
\item traversability remains the primary optional soft obstacle aid,
\item IMU safety is expected,
\item recovery is limited,
\item reverse behavior is restricted in marathon mode,
\item health-gate checks are encouraged,
\item and leaving the routed corridor becomes a stop condition.
\end{itemize}
...
\section{Why The Route Corridor Guard Was So Important}
Of all the additions, the route corridor guard deserves special emphasis.

Before this addition, OSM routing helped choose where the rover ought to go, but there was no direct runtime check saying: \emph{you are drifting too far away from the sidewalk route, stop now}. That gap mattered because the organizers' main safety concern was not abstract navigation quality. It was real-world path discipline.
...
\section{Why We Added A Preflight Validator Instead Of More Fancy Perception}
...
\section{What We Deliberately Did Not Add}
...
This was a deliberate engineering choice: \textbf{prefer smaller, better-justified safety additions over speculative autonomy expansion}.

\section{How To Use The New Pieces}
...
\section{What I Would Tell The Team Clearly}
...
\begin{itemize}[leftmargin=*]
\item a route-corridor stop to keep the rover near the routed sidewalk path,
\item battery warnings and optional battery stop behavior,
\item and a proper preflight validator.
\end{itemize}
...
\item Run the preflight validator first.
\item Use the conservative marathon command built on mission mode, LogoNav, OSM routing, traversability, and ultra-marathon mode.
\item Do not enable extra optional layers by default unless they are revalidated immediately before the event.
\item Confirm with the organizers whether the seven runs arrive as one mission or several.
\item Treat operator intervention as part of the intended safety workflow, not as a failure of principle.
\end{enumerate}
...
\section{Detailed Engineering History Of The Later Marathon Work}
...
\subsection{Repository And Path Confusion Had To Be Resolved First}
...
\subsection{The Model Audit Was Broader Than The First Story Section Suggests}
...
\begin{itemize}[leftmargin=*]
\item There was no YOLO-based object detector wired into the outdoor runtime.
\item The semantic segmentation path existed and was originally based on SegFormer with an ADE20K-oriented model id.
\item The monocular depth path existed through Depth Anything V2.
\item The traversability layer used the depth output to create a local blocked-vs-open steering signal, but it was not a full semantic understanding layer.
\item The main outdoor local controller remained LogoNav, not MBRA.
\end{itemize}
...
\subsection{External Research Was Used To Reframe The Night Problem}
...
\begin{itemize}[leftmargin=*]
\item Night-time pedestrian detection is materially harder than daytime detection.
\item Monocular depth quality degrades in low light and glare conditions.
\item Road-scene segmentation models trained on urban datasets such as Cityscapes are structurally more relevant than a generic ADE20K semantic model for sidewalk-versus-road interpretation.
\end{itemize}
...
\subsection{Several Tempting But Overconfident Ideas Were Explicitly Rejected}
...
\subsection{Night-Safe Mode Became A Real Runtime Profile}
...
\begin{itemize}[leftmargin=*]
\item lamp-on behavior,
\item a vision-quality safety gate for dark, glare-heavy, or low-detail frames,
\item stronger GPS safety gating,
\item explicit use of the IMU safety path,
\item route-corridor guarding,
\item and tighter runtime defaults for night operation.
\end{itemize}
...
\subsection{The IMU Safety Path Required Real Calibration Work}
...
\subsection{Mission Startup And SDK Behavior Needed Additional Discipline}
...
\begin{itemize}[leftmargin=*]
\item indoor connectivity checks without mission mode,
\item outdoor preflight checks without actually committing to mission execution,
\item and the full mission-run flow with the SDK's expected mission handshake.
\end{itemize}
...
\subsection{The OSM Story Became Much More Complex Than ``Turn It On''}
...
\subsection{Waypoint Handoff Turned Out To Be One Of The Hardest Practical Problems}
...
\subsection{Controller Diagnostics From Live Logs Forced Several Changes To LogoNav Shaping}
...
\begin{itemize}[leftmargin=*]
\item The robot could crawl too slowly for too long, especially when the learned policy plus safety shaping reduced forward speed to an ineffective level.
\item The stuck detector originally only considered relatively large forward commands as ``real motion,'' so low-speed creeping could evade recovery logic.
\item Turn-priority logic could be too eager, especially when a waypoint was not truly near but the bearing error happened to be large.
\item After re-routing or waypoint changes, the controller could attempt to align to an obviously bad next point rather than maintaining monotonic progress.
\end{itemize}
...
\subsection{Traversability Had To Become More Than A Gentle Suggestion}
...
\subsection{The Terminal Output Had To Be Rewritten Because Debuggability Was Part Of Safety}
...
\subsection{The Saved-Log Calibration Idea Was Partly Possible And Partly Not}
...
\begin{itemize}[leftmargin=*]
\item the old outdoor deployment script and its design choices,
\item and the live logs produced during the new outdoor tests.
\end{itemize}
...
\subsection{What The Final Runtime Had Actually Become By The End Of This Phase}
...
\subsection{What Still Remains Unresolved Even After All Of That}
...
\subsection{What The Team Should Do Next}
...
\begin{itemize}[leftmargin=*]
\item collect more real outdoor logs from the current runtime,
\item specifically review waypoint handoff behavior, reroute triggers, and low-clearance segments,
\item improve curb and step handling rather than adding unrelated perception complexity,
\item confirm the exact mission format and checkpoint semantics from the organizers,
\item and keep the operator workflow explicit and disciplined.
\end{itemize}
...
\subsection{The Honest Summary Of The Full Story}
...
\section{Code-Verified Appendix: Exact Later Runtime Mechanics}
...
\subsection{Three Parser Defaults That Became Surprisingly Important}
...
\begin{itemize}[leftmargin=*]
\item routed intermediate waypoints needed a smaller handoff radius than mission checkpoints,
\item hard align-turn behavior had to be restricted to nearby or clearly pathological cases,
\item and OSM routes sometimes needed aggressive pruning of nearby behind-the-rover starter waypoints.
\end{itemize}

\subsection{What Ultra-Marathon And Night-Safe Really Do In Code}
...
\subsection{The Later Route-Corridor Logic Is Also More Nuanced Than The Early Story}
...
\subsection{The Align Gate Was Ultimately Made Target-Aware}
...
\subsection{Intermediate Waypoint Handoff Is Now Dynamic}
...
\subsection{Traversability Was Ultimately Promoted To Hard Local Override}
...
\subsection{Representative Failure Logs That Drove The Later Fixes}
Several later fixes are easiest to remember through the characteristic logs that exposed them:
\begin{itemize}[leftmargin=*]
\item \texttt{[IMU EMERGENCY] imu\_emergency(tilt=89...)} while the robot was physically upright: this revealed that the original tilt logic was assuming the wrong rest-frame orientation.
\item \texttt{route\_corridor\_stop dist=... threshold=... ticks=3} repeated over and over: this exposed the stale-route deadlock and motivated reroute-from-current-pose.
\item \texttt{intermediate waypoint reached ...} followed immediately by an \texttt{ALIGN} block to a bad next point: this exposed the handoff and behind-waypoint problem.
\item \texttt{CP=... dist=... lin=+0.07...} with almost no net progress: this exposed the low-speed creeping regime and the mismatch with stuck detection.
\end{itemize}
...
\subsection{One Honest Technical Limitation Still Remains}
