...
class DepthDecoder_camera_ada3(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_bins=32, num_output_channels=1, use_skips=True):
        ...
        for s in self.scales:
            self.convs[("dispconv_f", s)] = Conv3x3(self.num_ch_dec[s], self.num_output_channels)
            self.convs[("dispconv_b", s)] = Conv3x3(self.num_ch_dec[s], self.num_output_channels)
        ...
        self.convs[("linear1_c")] = nn.Linear(512, 256)
    ...
    def forward(self, input_features):
        ...
        for i in range(4, -1, -1):
            ...
            if i in self.scales:
                self.outputs[("disp_f", i)] = self.sigmoid(self.convs[("dispconv_f", i)](x))
                self.outputs[("disp_b", i)] = self.sigmoid(self.convs[("dispconv_b", i)](x))
        ...
        #camera_hight = self.sigmoid(x5[:,2*self.num_bins + 2:2*self.num_bins + 3])
        ...
        #return self.outputs, camera_param, bin_width, camera_range, camera_hight
