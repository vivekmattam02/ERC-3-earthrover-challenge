...
Do not try to say all of this out loud.
The slide deck is intentionally simple.
This file is for:
...
"This is a short update on the ERC-3 EarthRover project. I’ll focus on indoor, outdoor, and marathon, and for each one I’ll explain what we chose, why we chose it, what worked, and what failed."
...
- determine which step/checkpoint comes next
...
Say:
"The system improved a lot, but the remaining issue is repeatability and edge-case handling, not lack of a direction. We now understand the failure modes much better."
...
### Short spoken version
"For outdoor, we kept LogoNav as the main controller, used OSM route expansion, and added safety layers around the runtime, because the outdoor problem was more about runtime stability than replacing the controller."
...
### Short spoken version
"For outdoor, one run reached all checkpoints. The others were around a 50 percent success rate overall, and three rounds failed."
...
### What worked
- one full run proves the pipeline can complete
...
### Short spoken version
"For the marathon, we kept the same outdoor base runtime and made it stricter, because the marathon problem is really about safe supervised completion."

### Technical explanation
The marathon framing changed the objective from:
- normal outdoor navigation

to:
- long-duration supervised operation with less tolerance for brittle behavior

That is why the work emphasized:
- IMU safety
- route corridor guard
- rerouting from live GPS
- waypoint handling cleanup
- better operator logs

### Key point
The marathon work was not about inventing a new algorithm.
It was about wrapping the existing runtime in stronger discipline.
...
### Short spoken version
"In the marathon run, the robot reached one checkpoint, then started spinning and toppled."
...
- this created repeated turning / align behavior
...
### Short spoken version
"So the short summary is: indoor is now best understood as MBRA-first checkpoint-step navigation, and we reached 8 out of 11 checkpoints. Outdoor is still best handled as LogoNav plus routing plus safety layers, with one full success and the others around 50 percent. Marathon showed that the biggest remaining issue is stability after checkpoint and waypoint transitions."
...
### Q: Why MBRA indoors?
Because indoor has strong corridor structure already. MBRA works best as the short-horizon local controller once exact checkpoint-step targets and graph progression define the overall motion structure.
...
### Q: Why not unify indoor and outdoor?
...
### Q: What do you think improved the project most?
